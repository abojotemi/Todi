      ...
      <div className="max-w-[1200px] mx-auto px-16 sm:px-6 max-sm:px-4">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6" role="list">
          {services.map((service) => (
            <li
              key={service.title}
              className="group bg-brand-surface-container-lowest border border-brand-secondary/15 rounded-2xl overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-primary/10 transition-all duration-300 flex flex-col"
            >
              ...
              <div className="p-8 flex flex-col flex-grow">
                <div
                  className={`w-14 h-14 rounded-xl bg-gradient-to-br ${service.accent} flex items-center justify-center mb-6 ${service.iconColor} group-hover:scale-105 transition-transform duration-300`}
                >
